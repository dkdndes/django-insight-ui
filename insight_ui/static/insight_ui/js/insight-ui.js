...
(function() {
  ...
  InsightUI.Sidebar = {
    init: function() {
      ...
      // Initialisiere Auto-Hide für linke Sidebar
      InsightUI.Sidebar.initAutoHide();
    },

    initAutoHide: function() {
      const leftSidebarButtons = document.querySelectorAll('[data-insight-toggle="left-sidebar"]');
      let isWheelPressed = false;
      let hideTimeout = null;
      let isMouseNearSidebar = false;
      
      // Überwache mittlere Maustaste
      ...
      document.addEventListener('mouseup', function(e) {
        if (e.button === 1) { // Mittlere Maustaste
          ...
          e.preventDefault();
          // Starte Auto-Hide Timer wenn Maus nicht in der Nähe ist
          if (!isMouseNearSidebar) {
            InsightUI.Sidebar.scheduleAutoHide();
          }
        }
      });

      // Überwache Mausposition
      document.addEventListener('mousemove', function(e) {
        const mouseX = e.clientX;
        const sidebarWidth = 256; // 16rem = 256px
        const proximityThreshold = 50; // Pixel-Abstand zur Sidebar
        
        // Prüfe ob Maus in der Nähe der linken Sidebar ist
        isMouseNearSidebar = mouseX <= sidebarWidth + proximityThreshold;
        
        if (isMouseNearSidebar || isWheelPressed) {
          // Zeige Sidebar wenn Maus nah ist oder Wheel gedrückt
          InsightUI.Sidebar.showLeftSidebar();
          InsightUI.Sidebar.cancelAutoHide();
        } else {
          // Verstecke Sidebar nach Verzögerung wenn Maus weit weg und Wheel nicht gedrückt
          InsightUI.Sidebar.scheduleAutoHide();
        }
      });

      // Zeige Sidebar initial
      InsightUI.Sidebar.showLeftSidebar();
    },
    ...
    scheduleAutoHide: function() {
      ...
      hideTimeout = setTimeout(function() {
        InsightUI.Sidebar.hideLeftSidebar();
    ...
    showLeftSidebar: function() {
      ...
      if (leftSidebar) {
        utils.removeClass(leftSidebar, 'closed');
        utils.removeClass(body, 'left-sidebar-closed');
    ...
    hideLeftSidebar: function() {
      ...
      if (leftSidebar) {
        utils.addClass(leftSidebar, 'closed');
        utils.addClass(body, 'left-sidebar-closed');
